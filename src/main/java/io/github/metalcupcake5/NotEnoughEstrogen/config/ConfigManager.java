...
public class ConfigManager {
    ...
    public static final Path CONFIG = FabricLoader.getInstance().getConfigDir().resolve("notenoughestrogen.json");
    ...
    public static void saveConfig() {
        if (!Files.exists(CONFIG)) {
            try {
                ...
                NotEnoughEstrogenClient.LOGGER.error("Couldn't create the config file");
                return;
            }
        }
        ...
        try {
            ...
            NotEnoughEstrogenClient.LOGGER.error("Couldn't save the config file");
        }
    }

    public static void loadConfig() {
        if (!Files.exists(CONFIG)) {
            NotEnoughEstrogenClient.LOGGER.info("could not find the config file, creating one");
            saveConfig();
            return;
        }
        boolean fix = false;
        try {
            ...
            NotEnoughEstrogenClient.LOGGER.warn("Malformed Json, Fixing");
            fix = true;
        } catch (IOException e) {
            NotEnoughEstrogenClient.LOGGER.error("Couldn't read the config");
        }
        if (fix)
            saveConfig();
    }
